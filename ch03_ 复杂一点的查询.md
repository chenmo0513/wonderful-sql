...
```
```sql
CREATE VIEW viewpractive5_1 (product_name, sale_price,regist_date)
AS
...
```
INSERT INTO ViewPractice5_1 VALUES (' 刀子 ', 300, '2009-11-02')
> 1146 - Table 'shop.viewpractice5_1' doesn't exist
> 时间: 0.037s
...
SELECT
	...
	SELECT
		avg_price 
	FROM
		( SELECT product_type, AVG( sale_price ) AS avg_price FROM product GROUP BY product_type ) AS p2 
	WHERE
...
average 不会出现 NULL， 其余的会出现，所以答案为不正确。
...
```
![](E:\Documents\GitHub\wonderful-sql\img\ch03\3-6-1.png)
...
![3-9](./img/ch03/3-9.png)

```sql
SELECT
	COUNT( CASE WHEN sale_price <= 1000 THEN sale_price ELSE NULL END ) 'low_price',
	COUNT( CASE WHEN sale_price BETWEEN 1001 AND 3000 THEN sale_price ELSE NULL END ) 'mid_price',
	COUNT( CASE WHEN sale_price >= 3000 THEN sale_price ELSE NULL END ) 'high_price' 
FROM
	product;
```
