...
--  主要是因为子查询的时候，查询的是所有字段，而不是单个字段导致出现1241 - Operand should contain 1 column(s)的问题：
...
-- SELECT max(sale_price) as Max_price, product_type,product_id,product_name
-- 	FROM (SELECT * FROM product UNION  SELECT * FROM product2) as P GROUP BY product_type;


-- 商店有售

SELECT P.product_type, P.product_name, sp.shop_name, P.m_price,p.product_id
	FROM shopproduct AS SP
	LEFT JOIN (SELECT product_type, product_id, product_name, MAX( sale_price ) AS m_price 
							FROM product 
...
SELECT product_id, product_name, sale_price,(SELECT sum(sale_price) FROM product as p2) as cum
FROM product
