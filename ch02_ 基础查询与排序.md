...
由于选取的列不同，所以返回的列不同，**where可以不是显示的列**
...
**希望选取NULL记录时，需要在条件表达式中使用IS NULL运算符。希望选取不是NULL的记录时，需要在条件表达式中使用IS NOT NULL运算符。 IS NOT 中间有空格。**
...
1. WHERE 要在 GROUP BY 之前 “SELECT ➡️	 2. FROM ➡️	 3. WHERE ➡️	 4. GROUP BY” 先筛选，后分组

2. 括号（）使用不是半角， product_name 不是数值列

3. 因为product_id是key，所以此处select与group by 不一致没有问题。
